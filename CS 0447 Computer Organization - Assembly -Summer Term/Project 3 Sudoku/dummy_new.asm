...
# http://www.kernel-panic.it/software/sudokiller/sudokiller.s.html
# This is the game board. Fill it with the puzzle you want to solve
board:      .byte     0, 0, 5, 4, 0, 0, 7, 0, 6
            .byte     0, 0, 0, 6, 5, 3, 0, 8, 2
            .byte     0, 0, 0, 0, 0, 0, 5, 0, 0
            .byte     2, 0, 0, 1, 7, 0, 0, 0, 4
            .byte     9, 0, 0, 0, 0, 0, 0, 0, 5
            .byte     3, 0, 0, 0, 8, 5, 0, 0, 7
            .byte     0, 0, 1, 0, 0, 0, 0, 0, 0
            .byte     8, 6, 0, 5, 3, 2, 0, 0, 0
            .byte     5, 0, 2, 0, 0, 4, 6, 0, 0
...
################################################################################
main:
    li		$a0, 0xffff8000            # Offset of first cell to guess
...
guess:
    ...
    lb      $t0, 0($s0)   # Load current cell's value in $t0
...
guess_loop:
    ...
    sb      $s3,  0($s0)   # If OK, assign number to cell
...
guess_chk_failed:
    add     $s3, $s3, -1                # Decrement the number to test
    bnez    $s3, guess_loop       #   and test it
    sb      $zero,  0($s0) # If no number worked, empty the cell
...
check_row:
    lb      $t2,  0($t1)   # Value in the current cell
...
check_col:
    lb      $t2,  0($t1)   # Value of the current cell
