...
public class subset
{
	...
	public subset()
	{
		...
		states = 0;
		found = false;
		// This loop makes the initial call to the recursive subset sum algorithm.
		// Each call starts the subset with the next item in the array.  The remaining
		// items are then considered recursively.
		for (int i = 1; i <= N; i++)
		{
			states++;
			if (size[i] <= M)
		...
		// solution to the subset sum problem.   Note that the time for this algo
		// can easily be predicted: Theta(NM).  This seems polynomial, but recall that
		// M is the sum of a subset, whose items can be arbitrarily large. Thus, the
		// algorithm can in fact be slower than the branch and bound algorithm in some
		// cases.  Consider file subset4.dat and you can see where the dynamic programming
		// algorithm falters.  Also remember that the dynamic programming algorithm requires
		// Theta(M) memory, and this certainly can be a problem for large M as well.
