...
public final class ArrayBag<T> implements BagInterface<T> {
	...
	public boolean splitInto(BagInterface<T> first, BagInterface<T> second) {
		checkInitialization();
		boolean result = true;
		int secondPart = numberOfEntries/2;
		int firstPart = numberOfEntries - secondPart;
		first.clear();
		second.clear();

		for(int i=0; i< firstPart; i++) {
			result = first.add(bag[i]);
			if(!result)
				return result;
		}
		for(int i=0; i< secondPart; i++) {
			result = second.add(bag[i+firstPart]);
			if(!result)
				return result;
		}
		return result;
	...
	public boolean addAll(BagInterface<T> toAdd) {
		...
		if(toAdd.getCurrentSize() + numberOfEntries 
				<= bag.length) {
			T[] array = toAdd.toArray();
			for(T entry : array) {
				add(entry);
			}
			result = true;
